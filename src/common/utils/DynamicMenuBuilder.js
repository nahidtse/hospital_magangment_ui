...
export class DynamicMenuBuilder {
    ...
    static build(rolePermissionData) {
        ...
        // 1. all menu + parent recursively add 
        rolePermissionData.forEach(item => {
            if (!item.menu) return;

            this.addMenuWithParents(item.menu, item.permissions || [], allMenus);
        });

        // 2. Parent-Child hierarchy Create
        ...
        // 3. Root menu Find
        const topLevelMenus = Array.from(allMenus.values()).filter(menu => 
            menu.is_top_menu === 1 || !menu.parent_menu_id
        );

        // 4. Frontend structure Create
        return this.buildNestedStructure(topLevelMenus);
    }

    // Recursive function: menu + all parents add
    static addMenuWithParents(menu, grantedPermissions, allMenus) {
        if (!allMenus.has(menu.id)) {
            allMenus.set(menu.id, {
                ...menu,
                grantedPermissions,
        ...
        // IF parent Still
        if (menu.parent_menu) {
            if (!allMenus.has(menu.parent_menu.id)) {
                allMenus.set(menu.parent_menu.id, {
                    ...menu.parent_menu,
                    grantedPermissions: [], // Parent general container
                    children: []
                });
            }
            // Recursively add parents
            this.addMenuWithParents(menu.parent_menu, [], allMenus);
        }
    }
    ...
    static buildNestedStructure(menus) {
        ...
        return sortedMenus.map(menu => {
            ...
            const frontendItem = {
                ...
                type: hasChildren ? 'sub' : 'link',
                menusub: hasChildren,
                permissions: (menu.grantedPermissions || []).map(p => {
                    const normalized = String(p.permission_name).toLowerCase();
                    const parts = normalized.split('_');
                    return parts[parts.length - 1]; // create | edit | view
                }),
                module_id: menu.module_id,
                sort_order: menu.sort_order || '999',
                active: false,
    ...
    static getIcon(menu) {
        if (menu.is_top_menu !== 1) return '';
        const iconMap = {
            'Supper Admin': 'fe-lock',
            'Dashboard': 'fe-home',
            'Appointment': 'fe-calendar',
            'Doctor\'s': 'fe-briefcase',
            'Setting': 'fe-settings',       // Settings
            'Outdoor': 'fe-compass', 
        };
        return iconMap[menu.menu_name] || 'fe-file';
    }
    ...
    static generatePath(menu) {
        const rawRoute = menu.route_name;

        const base = String(import.meta.env.BASE_URL || '/').replace(/\/$/, '');

        return `${base}/${rawRoute}/dataTable`; 
    }
}
