...
export const COLUMNS = [
    ...
    {
        Header: (<div className="text-center">{"Opening Date"}</div>),
        accessor: "openingDate",
    },
    {
        Header: (<div className="text-center">{"Opening Balance"}</div>),
        accessor: "openingBalance",
    },
    {
        Header: (<div className="text-center">{"Route No"}</div>),
        accessor: "route",
    },
    {
...
export const DATATABLE = (bankAccount, handlers) =>
    bankAccount.map((account, id) => ({
        id: id + 1,
        accountName: account.ac_name || "Account Name",
        accountNumber: account.ac_number || "Account Number",
        bankName: account.afm_bank.bank_name || "Bank Name",
        branch: account.branch_name || "Branch Name",
       openingDate: account.opening_date && !isNaN(new Date(account.opening_date))
                    ? format(new Date(account.opening_date), 'dd-MM-yyyy')
                    : "Open Date",
        openingBalance: (<div className="text-end">{account.opening_balance || "0.00"}</div>),
        route: account.route_no || "",
        status: account.status || "Status",
        action: (
            <>
                <Link to={`${import.meta.env.BASE_URL}bankaccount/singledata/${account.id}`} >
                    <i className="bi bi-eye btn-sm bg-info"></i>
                </Link>
                <Link to={`${import.meta.env.BASE_URL}bankaccount/edit/${account.id}`} state={{bankAccount}}>
                    <i className="bi bi-pencil btn-sm bg-primary ms-2"></i>
                </Link>
                <span onClick={() => handlers.deletePermissionAlert(account.id)} className="btn-sm bg-danger ms-2" style={{ cursor: "pointer" }}>
...
export const BasicTable = () => {
    ...
    const handleDeleteClick = async (accountId) => {
        try {
            const result = await fetch(`${baseURL}/bank_account/destroy/${accountId}`, {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json',
            ...
            if (response.status == 'success') {
                setBankAccount(prevContact => prevContact.filter(c => c.id !== accountId));
    ...
    const dataTable = useMemo(() => DATATABLE(bankAccount, {
                ...
                <Col xl={12}>
                    <Card className="custom-card">
                        <Card.Header className="justify-content-between">
                            <div className='card-title'>Bank Account List</div>
                            <div className="prism-toggle">
                                <Link to={`${import.meta.env.BASE_URL}bankaccount/createform`} state={{bankAccount}}><button
                                    type="button"
                        ...
                        <Card.Body>
                            ...
                            <div className="d-block d-sm-flex mt-4 ">
                                <span>
                                    ...
                                    {Math.min((pageIndex + 1) * pageSize, bankAccount.length)} of{" "}
                                    {bankAccount.length} entries
                                </span>
