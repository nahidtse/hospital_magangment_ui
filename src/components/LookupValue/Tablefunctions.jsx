...
export const DATATABLE = (contacts = [], handlers) =>
    contacts.length > 0 ? contacts.map((contact, id) => ({
        ...
        lookupcode: contact.lookup_type.lookup_type  || "Lookup Code",
        // lookupcode: contact.lookup_type.lookup_type + "  (" + contact.lookup_type.lookup_code + ")" || "Lookup Code",
...
export const BasicTable = () => {
    ...
    const handleDeleteClick = async (lookupValueId) => {
        try {
            ...
            if (response.status == 'success') {
                setContacts(prevContact => prevContact.filter(c => c.id !== lookupValueId));
    ...
    const handleEditDataById = (contact) => {
        // const data = contacts.find((contact) => contact.id == lookupValueId);
        setShowData(true);
        setSingleData(null)
                                ...
                                <Card.Body>
                                    ...
                                    <div className="d-block d-sm-flex mt-4 ">
                                        <span>
                                            ...
                                            {Math.min((pageIndex + 1) * pageSize, contacts.length)} of{" "}
